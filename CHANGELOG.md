...
## 2.0301 (2026-09-23)
### 新增
- 【骰子】暗骰 `.rh` / `/rh`：结果私聊发送，群内只回一句提示
- 【骰子】暗骰私聊绑定 `.rhbind` / `/rhbind`：私聊里拿 6 位短码 → 群里发「绑定私聊 <短码>」→ 之后 `.rh` 可送达。
  原因是 QQ 官方 Bot 的群消息只能拿到 member_openid，而单聊投递要 user_openid，QQ 不提供两者映射
- 【骰子】人物卡属性系统 `.st` / `/st`：录入 / 查看 / 增减 / 删除 / 清空，支持 `.st 体力13 侦查-5` 混写
- 【骰子】`.ra` 技能值缺省时自动从人物卡取值，未录入会提示先录

### 变更
- 【骰子】人物卡与骰面设置改用归一化后的「群 + 用户」标识，两个平台共用同一张卡、同一份设置
- 【骰子】`.ra` 不带技能名时只回点数（此前技能值为 0 时 `judge_coc7th` 恒返回「成功」）

### 修复
- 【骰子】`/ra` `/dice set` `/dnd` 的参数整条残留：这几个命令去剥 `^/cmd` 前缀，但 AstrBot 的
  waking_check 阶段早就把唤醒前缀剥掉了，导致参数解析全部失效（`/dice set 20` 只回「当前骰子」、
  `/dnd 3` 数量被忽略、`/ra 侦查` 去卡里查一个叫「ra 侦查」的属性）
- 【骰子】`.st` 混写时静默丢弃无法识别的部分，现在会明确提示跳过了什么
- 【骰子】`.st` 增减改用均匀分布，不再走伪平均（掉血 1d3 必须能掷出 1 和 3）
- 同步 metadata.yaml 与 main.py 版本号（此前 2.0207 / 2.0204 不一致，会导致 AstrBot 拉不到更新）
...
- 签到回复在 QQ 官方 Bot 平台改用 Markdown 模板消息（群聊不支持 Embed 渲染）
- fanqie 的 `force` 和 `reset` 调整为仅管理员可用，`add/del/list/get_umo` 开放给所有用户
- 帮助文本按权限重新分组
- WebUI 新增 `image_url` 配置项，可自定义签到图片链接

### 文档
- README 全面更新指令表和新增 Markdown 签到说明
