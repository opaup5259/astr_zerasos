...
## 2.0302 (2026-09-23)
### 修复
- 【骰子】`.st` 贴整张卡录入失败：解析器要求每个「属性名+数值」必须是独立空白 token，
  `力量45str45敏捷80` 这种连写整串会被判为无法识别。改为按「名字+数字」对扫描，
  空格与 `:` `=` 分隔符都不再必需
- 【骰子】`.st` 无法识别时会把用户输入整串回显（贴 500 字的卡就把整张卡丢了回来），现在截断到 40 字
- 【骰子】`.st` 输出改用 `<br />` 换行，与插件其它卡片输出保持一致

### 新增
- 【骰子】`.st.<角色名>-<属性...>` 支持角色名前缀（Dice! 风格），角色名存入卡内并在查看时显示。
  判据是看 `-` 右边：右边是纯数值/骰点则仍是增减（`.st hp-1d3`），否则左边是角色名
- 【骰子】补全 COC 技能别名：运气 / 信用评级 / 汽车驾驶 / 图书馆使用 / 锁匠 / 操作重型机械 /
  博物学 / 领航 / 克苏鲁神话 / 计算机使用 / 理智值

### 变更
- 【骰子】`.st` 单次录入超过 8 项时只回「已录入 N 项属性」，不再逐条列出撑爆消息
