...
class BqbManager:
    ...
    async def _analyze_image(self, image_url: str) -> list[str]:
        ...
        try:
            ...
            prompt = (
                "你是一个表情包分析专家。分析这张表情包图片，输出精准的标签。\n"
                "规则：\n"
                "1. 先读取图片中所有的文字（OCR），文字内容是最重要的信息\n"
                "2. 理解这个表情包在表达什么梗、什么情绪\n"
                "3. 判断表情包的风格类型\n"
                "4. 判断该表情包适合在什么聊天场景下使用\n"
                "\n"
                "输出5-8个标签，用英文逗号分隔。不要序号和说明。\n"
                "如果图片有文字，文字提取结果必须放在第一个标签。\n"
                "标签从以下维度选取：\n"
                "- 类型：沙雕/抽象/熊猫人/二次元/萌系/猫猫/狗狗/动漫/真人/表情包/其他\n"
                "- 画面主体：人物/动物/文字/物品/场景\n"
                "- 情绪：开心/震惊/无语/哭泣/生气/害怕/尴尬/得意/嘲讽/摆烂/破防/蚌埠住了/难绷/流汗/害羞\n"
                "- 适用场景：打招呼/再见/同意/拒绝/安慰/吐槽/催促/晚安/早上好/道歉/敷衍/拒绝回答/不知道\n"
                "- 如有知名角色或IP请标注，如蔚蓝档案/原神/崩铁等"
            )
