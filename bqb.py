...
class BqbManager:
    ...
    async def _analyze_image(self, image_url: str) -> list[str]:
        ...
        try:
            ...
            prompt = (
                "【身份】你是一个专业的表情包(meme/贴图)标注师，精通中文互联网梗文化。\n"
                "【任务】分析这张表情包图片，输出详细、准确、有用的标签，供后续根据聊天语境检索匹配。\n"
                "\n"
                "【输出格式】\n"
                "只输出一行标签，用英文逗号 , 分隔。不要序号、不要说明、不要换行。\n"
                "输出8-15个标签。\n"
                "\n"
                "【第一条规则——OCR优先】\n"
                "如果图片中有文字，第一个标签必须完整提取图片中的全部文字内容，用引号包裹。\n"
                "例如：\"卧槽！开摆！\"\n"
                "如果没有文字，第一个标签用[无文字]。\n"
                "\n"
                "【第二条规则——理解梗文化】\n"
                "- 判断这是不是一张常见的meme/梗图，如果是，标注对应的梗名称\n"
                "- 理解图片表达的核心情绪和态度（不仅仅是表面情绪，而是这个图在聊天中的实际用途）\n"
                "- 例如：摆烂、敷衍、无语、蚌埠住了、破防、难绷、流汗黄豆、差不多得了、典、急、乐、孝、赢麻了\n"
                "\n"
                "【标签维度】从以下维度尽可能多地覆盖：\n"
                "1. 文字内容（必须，见第一条规则）\n"
                "2. 风格类型：沙雕/抽象/熊猫人/二次元/萌系/三次元/动漫/真人截图/表情包/手绘/像素风/其他\n"
                "3. 画面主体：人物/动物/文字/物品/场景/表情/动作/其他\n"
                "4. 核心情绪（选最贴切的1-2个）：开心/震惊/无语/哭泣/生气/害怕/尴尬/得意/嘲讽/摆烂/破防/蚌埠住了/难绷/流汗/害羞/嫌弃/鄙视/疑问/恍然大悟/麻木/愤怒/喜悦/悲伤/恐惧/厌恶/轻蔑/惊讶/期待/失望/安心/焦虑\n"
                "5. 聊天用途（这个表情包一般在什么场景下发）：打招呼/再见/同意/拒绝/安慰/吐槽/催促/晚安/早安/道歉/敷衍/拒绝回答/不知道/阴阳怪气/自嘲/炫耀/认输/鼓励/疑问/催更/求求了/感谢\n"
                "6. 知名角色/IP/人物特征：如蔚蓝档案/原神/崩坏/崩铁/FGO/碧蓝档案/东方/Vtuber/猫猫/狗狗/熊猫人/熊猫头/金馆长/张学友/花泽香菜/其他知名角色\n"
                "7. 画面描述：简短的画面特征，如猫娘/歪头/比心/捂脸/捂嘴笑/流汗/翻白眼/竖中指/竖大拇指/摸头/抱抱/跪/趴/蹲/站/坐/跳/躺/指/瞪/眯眼/斜眼/举手\n"
                "\n"
                "【重要】标签一定要多维度覆盖，不要只写3-4个，尽量写8-12个。\n"
                "要让人看到标签就能大概知道这张图长什么样、表达什么情绪、适合什么场景。\n"
            )
    ...
    # ── CRUD ────────────────────────────────────
    async def remake_bqb_tags(self, num: int) -> bool:
        """重新分析指定表情包的标签"""
        for item in self.index:
            if item["id"] == num:
                ...
                if not os.path.exists(file_path):
                    return False
                tags = await self._analyze_image(file_path)
                item["tags"] = tags
                self._save_index()
                logging.info(f"[BQB] 重新打标 #{num}: {tags}")
                return True
        return False
